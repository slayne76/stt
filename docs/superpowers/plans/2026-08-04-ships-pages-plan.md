...
- **`byLevelProgressDesc` (key 2 above) is intentional, not dead/redundant code, even though it currently never changes ordering on this app's real pages.** `max_level` is 1:1 with `rarity` in the real data (every rarity-5 ship has `max_level: 9`, every rarity-4 ship has `max_level: 8`), and each page only shows one rarity — so within a page, a `level` tie is already a progress-fraction tie, and key 3 (missing schematics) is what actually breaks ties in practice. This is a deliberate, explicit user requirement (implement the general rule, not a page-specific shortcut) and is documented in the design spec. Do not remove, simplify away, or flag this comparator as unreachable/redundant — it is correct general logic that would matter the moment a page ever mixed rarities.
