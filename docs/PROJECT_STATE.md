...
client/src/
  ...
  dilemmas/                      Static dilemma-mission data logic + the Dilemmas page's table (see
    ...
    DilemmasTable.tsx            #/Name/Choices/Reward/Drop Rate table; no usePagination — the whole
                                 list renders directly (only 40 rows so far, see "Dilemmas page" below)
  pages/
...
to 40 via several same-day additions — all but one purely data-only
...
`project_stt-tracker-state.md` for the running list — 4 ship rewards
